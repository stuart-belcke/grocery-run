...
/* A FETCHED page rather than a pasted one — the same AllRecipes recipe, but
   the WHOLE document instead of the part somebody selected.

   WHY THIS IS A DIFFERENT FIXTURE AND NOT A TWEAK TO THE ONE ABOVE. A paste
   starts where the reader started selecting, so it never contains the site's
   navigation. A fetch always does. That single difference breaks the rule
   parseRecipeText used to find the list — "the first Ingredients heading" —
   because the nav menu has one, a prose explainer has one, and the recipe's
   own is third.

   Reconstructed from the text an iOS Shortcut actually fetched on a phone
   (Get Contents of URL -> Match Text), read off five screenshots, so the
   SHAPE is real and observed rather than imagined. It is trimmed: the real
   document also carries a footer, comments and nutrition boilerplate, none of
   which sits between the headings this exercises. Worth replacing with the
   captured text verbatim when there is a copy to hand. */
const ALLRECIPES_FETCHED = [
  ...
  "",
  "\t•\tPotatoes: Slice Russet potatoes into 1/4-inch (or thinner) slices. Use a mandoline slicer if you have one.",
...
test("parseRecipeText picks the real ingredient list out of a page with six Ingredients headings", () => {
  /* THE BUG A FETCH CREATES AND A PASTE NEVER COULD. Taking the first
     heading, this page's nav menu wins — and on this exact markup the very
     next line is ANOTHER heading, so the old rule broke instantly and
     returned ZERO ingredients: an import that silently comes back empty.
     Shift the menu slightly and the same rule instead imports Chicken, Beef
     and Pork as the shopping list. Both are the same defect.
     Scored on whether the lines carry AMOUNTS, the real list wins 7-0. */
  const result = parseRecipeText(ALLRECIPES_FETCHED);
  assert.equal(result.ingredients.length, 8, JSON.stringify(result.ingredients.map((i) => i.name)));
  assert.deepEqual(result.ingredients.map((i) => i.name), [
    "Medium russet potatoes",
    "Medium onion",
    "Salt and ground black pepper",
  ...
  for (const junk of ["Chicken", "Beef", "Pork", "Seafood", "View all"]) {
    assert.ok(!result.ingredients.some((i) => i.name === junk), `the navigation menu was imported: ${junk}`);
  }
});

test("a fetched page still yields its servings and its steps", () => {
  // The rest of the record has to survive the heading change too — the list
  // being right is no good if the page's own title became an ingredient or
  // the photo credits came back as steps.
  const result = parseRecipeText(ALLRECIPES_FETCHED);
  assert.equal(result.servings, 4);
  assert.equal(result.name, "Creamy au Gratin Potatoes Recipe");
  const steps = result.notes.split("\n").filter(Boolean);
  assert.equal(steps.length, 3, JSON.stringify(steps));
  assert.ok(!result.notes.includes("Dotdash Meredith"), "a photo credit became a step");
});
