...
/* ---------------- item 30: which build last wrote the household ----------------
   ...
   The complaint this answers: two phones diverged and the only signal either
   gave was "Synced", which says the socket is connected and nothing about
   whose data won. Settings already showed the build THIS device runs; this is
   the other half of that pair.

   NOT A PROMPT, and the first version of this item was — a modal on the newer
   phone telling its user to go and ask someone else to update. The app already
   offers every device its own "Update available — Reload now", so that was
   both redundant and unactionable. What is tested here is a line you go and
   READ when something already looks wrong. */

test("SHOULD: name the build that last wrote the household, when it isn't this one", async () => {
  const catalog = smallCatalog();
  catalog.buildId = "2019-01-01 00:00 UTC · 0ldb111";
  const page = await openApp(BASE, { catalog });
  try {
    await page.tab("Settings");
    const text = await page.textContent("body");
    assert.match(text, /Household last saved by 2019-01-01 00:00 UTC · 0ldb111/, "the household's own build stamp should be shown");
    // And the pair is the point: this device's build is still named too, or
    // there is nothing to compare the line above against.
    assert.match(text, /Build \S/, "this device's own build should still be shown alongside it");
    assertNoPageErrors(page, assert);
  } finally {
    await page.done();
  }
...
test("SHOULD: say nothing when the household was last written by this same build", async () => {
  /* Hidden when it matches, which is nearly always — a line that is normally
     noise stops being read, and then it is not there for the one time it
     matters. Asserted by taking the build from the page itself rather than
     hardcoding one, so this cannot pass by naming a stamp nothing uses. */
  const page = await openApp(BASE, { catalog: smallCatalog() });
  try {
    await page.tab("Settings");
    // Read the Build paragraph itself rather than scraping the whole body:
    // the stamp contains spaces, so a loose \S+ run off body text picks up a
    // near-miss and the comparison silently tests nothing.
    const line = await page.locator("p").filter({ hasText: /^Build / }).first().innerText();
    const mine = line.replace(/^Build\s+/, "").trim();
    assert.ok(mine, `fixture check: Settings should name this device's build, got ${JSON.stringify(line)}`);
    await page.done();

    const catalog = smallCatalog();
    catalog.buildId = mine;
    const same = await openApp(BASE, { catalog });
    try {
      await same.tab("Settings");
      assert.doesNotMatch(await same.textContent("body"), /Household last saved by/, "a matching build should say nothing at all");
      assertNoPageErrors(same, assert);
    } finally {
      await same.done();
    }
  } catch (e) {
    await page.done().catch(() => {});
    throw e;
  }
});
